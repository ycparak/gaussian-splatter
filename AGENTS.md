...
- The app renders a full-screen Three.js particle scene. The entrypoint is `src/main.js`, which mounts `src/core/Three.js` into `#app`.
- Core rendering lives in `src/core`:
	- `WebGLContext.js` owns the singleton renderer/canvas setup and resize behavior.
	- `Three.js` wires the render loop, scene, post-processing, and resize events.
	- `PostProcessing.js` configures the composer, bloom pass, color adjustment pass, and output pass.
- Scene composition lives in `src/scenes/Scene.js`. It sets up the camera, camera rig, black fogged scene, PLY particle loader, and loader progress UI.
- Particle loading and simulation live in `src/utils/PlyLoader.js`. It fetches a binary PLY file, parses positions/colors, initializes `GPUComputationRenderer`, and drives the particle shader uniforms.
- Shaders live in `src/shaders`, including the GPGPU particle simulation shader at `src/shaders/gpgpu/particles.glsl`.
- The point-cloud asset is `public/tokyo.min.ply`. The helper script `scripts/strip-ply.mjs` can strip larger PLY files down to the fields this app uses and emit a gzipped copy.
- Vite is configured in `vite.config.js` with Tailwind CSS and GLSL plugins. The production `base` is `/three-ml-sharp/`, so be careful changing asset paths.
...
- Use JavaScript ES modules. Keep imports/extensions consistent with the existing Vite setup.
...
- Keep Three.js lifecycle responsibilities separated:
	- renderer/canvas work belongs in `WebGLContext.js`;
	- animation loop and top-level orchestration belong in `Three.js`;
	- scene objects and camera setup belong in `Scene.js`;
