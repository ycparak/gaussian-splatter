...
export function useRecordingSession(threeRef: RefObject<Three | null>) {
	...
	const [secondsRemaining, setSecondsRemaining] = useState(RECORDING_DURATION_SECONDS)
	...
	const start = useCallback(() => {
		...
		stopTimeoutRef.current = window.setTimeout(stop, RECORDING_DURATION_SECONDS * 1000)
