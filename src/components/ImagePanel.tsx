...
import { bundledScenes } from "@/src/scenes/availableScenes";
		...
		<>
			...
			{selectedFile ? (
				<Button
					...
					disabled={isBusy}
					icon={
						isBusy ? <LoaderCircle className="animate-spin" /> : <Sparkles />
					}
					onClick={onGenerate}
				>
...
function StatusLine({
	...
	return (
		<div
			className={cn(
				"flex min-h-8 items-center gap-2 rounded-[8px] bg-neutral-800/50 px-3 text-[11px] leading-4 text-neutral-400",
...
function SceneList({
	...
	return (
		<ScrollArea className="max-h-52 pr-0">
			{scenes.length === 0 ? (
				<div className="rounded-[8px] bg-neutral-800/50 px-3 py-2 text-[11px] text-neutral-400">
