...
export default function ImagePanel({
	...
	async function followGenerationJob(initialJob: GenerationJob, signal: AbortSignal) {
		...
		<>
			...
			<button
				...
				className={cn(
					...
					isDragging && 'bg-cyan-400/10 text-neutral-200 ring-1 ring-cyan-300/70'
				)}
...
function StatusLine({
	...
	const isRunning = status === 'queued' || status === 'running' || status === 'optimizing'
...
function SceneListItem({
	...
	const bundledPercent = scene.source === 'bundled' ? bundledPlyPercentById[scene.id] : null
	...
	return (
		<button
			...
			{bundledPercent ? (
				<span className='shrink-0 text-[11px] text-neutral-400'>{bundledPercent}% of ply</span>
			) : null}
		</button>
	)
}
