...
import type { GeneratedScene, GenerationJob, SceneManifest } from '../shared/types'
...
async function createSceneJob(request: Request): Promise<Response> {
	...
	jobs.set(id, job)
	processJob(job, { inputDir, outputDir, sceneDir, inputPath }).catch((error: unknown) => {
		const normalizedError = error instanceof Error ? error : new Error(String(error))
		updateJob(job, {
			status: 'error',
...
async function processJob(
	...
	const sharpResult = await runSharp(paths.inputDir, paths.outputDir).catch((error: unknown) => {
...
async function runSharp(inputDir: string, outputDir: string) {
	const proc = Bun.spawn([SHARP_BIN, 'predict', '-i', inputDir, '-o', outputDir], {
		cwd: ROOT,
		stdout: 'pipe',
		stderr: 'pipe',
	})
...
async function findFirstFile(directory: string, extension: string): Promise<string | null> {
...
async function serveGenerated(pathname: string): Promise<Response> {
	...
	if (filePath !== GENERATED_ROOT && !filePath.startsWith(`${GENERATED_ROOT}/`)) {
